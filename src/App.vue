<template>
        <div id="app">
            ...
            <input type="button" @click = "outputSummaryResult" class="menu" value="カテゴリ別サマリ出力" title = "カテゴリ別に経過時間を集約し、その結果を出力します。" />
            ...
            <div id="settings" v-show="settings.showSettings">
                <h2>設定</h2>
                <h3>カテゴリ設定（1行に1項目）</h3>
                <textarea @change="onChangeCategoryStr" v-model = "categoryStr"
                 rows="10" cols="80"></textarea>
                <br />
                <br />
                <h3>タイムライン出力テンプレート</h3>
                <p>ヘッダ<br />
                <input typeop = "text" v-model = "outputTemplate.header" @change = "onChangeOutputTemplate" size = "80" title = "出力の一番最初に1度だけ出力される文字列" /></p>
                <p>本文<br />
                <input type = "text" v-model = "outputTemplate.body" size = "80" @change = "onChangeOutputTemplate" title = "各項目ごとに出力される文字列" /></p>
                <p>フッタ<br />
                <input type = "text" v-model = "outputTemplate.footer" size = "80" @change = "onChangeOutputTemplate" title = "出力の一番最後に1度だけ出力される文字列" /></p>
                ...
                <p>
                    ヘッダ<br />
                    <input typeop = "text" v-model = "outputTemplate.summaryHeader" @change = "onChangeOutputTemplate" size = "80" title = "出力の一番最初に1度だけ出力される文字列" />
                </p>
                <p>
                    本文<br />
                    <input type = "text" v-model = "outputTemplate.summaryBody" size = "80" @change = "onChangeOutputTemplate" title = "各項目ごとに出力される文字列" />
                </p>
                <p>
                    フッタ<br />
                    <input type = "text" v-model = "outputTemplate.summaryFooter" size = "80" @change = "onChangeOutputTemplate" title = "出力の一番最後に1度だけ出力される文字列" />
                </p>
                ...
                <div style="margin-top:1em;align-content:center;">
                        ...
                        <input type="checkbox" v-model="settings.isSummaryFirstInAllOutput" @change="onChangeSettings" id="switchIsSummaryFirst"/>
                        <label class="switch" for="switchIsSummaryFirst"><div></div></label>
                </div>
...
export default {
  ...
  methods: {
      ...
      replaceItem: function (upperIndex, lowerIndex) {
          ...
          if (upperItemStartTime != "" && lowerItemDurationTime != "") {
              ...
              if (upperItemDurationTime != "") {
                  tempDate = this.convertTimeStrToDate(this.scheduleList[lowerIndex].start);
      ...
      getDurationTime: function (index) {
          ...
          var startDate = this.convertTimeStrToDate(this.scheduleList[index].start);
          var endDate = this.convertTimeStrToDate(this.scheduleList[index].end);
          if (startDate > endDate) {
              endDate.setDate(endDate.getDate() + 1);
          }
          var duration = Math.floor((endDate - startDate) / 1000 / 60);
          var durationHour = ("0" + (Math.floor(duration / 60)).toString()).slice(-2);
          var durationMin = ("0" + (duration % 60).toString()).slice(-2);
      ...
      replaceOutputStr: function (item, template) {
          var startStrArray = this.scheduleList[item].start.split(':');
          var endStrArray = this.scheduleList[item].end.split(':');
          var durationStrArray = this.scheduleList[item].duration.split(':');
          var lineStr = template
...
<style>
  ...
  input[type="checkbox"] + label.switch > div {
      position: absolute;
      top: 1px;
      left: 2px;
      width: 12px;
  ...
  input[type="checkbox"]:checked + label.switch > div {
